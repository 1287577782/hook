...
from mouse_event import ButtonEvent, WheelEvent, MoveEvent, LEFT, RIGHT, MIDDLE, X, X2, UP, DOWN, DOUBLE, WHEEL, HORIZONTAL, VERTICAL
import time
import atexit
import win32con
LPMSG = POINTER(MSG)
...
def listen(queue):
    def low_level_mouse_handler(nCode, wParam, lParam):
        ...
        if nCode == win32con.HC_ACTION:
            ...
            param=ctypes.cast(lParam,MSLLHOOKSTRUCT_p)
            
        #鼠标左键点击
        if wParam == win32con.WM_LBUTTONDOWN:
            print("左键点击，坐标：x:%d,y:%d" % (param.contents.x,param.contents.y))
        elif wParam == win32con.WM_LBUTTONUP:
            print("左键抬起，坐标：x:%d,y:%d" % (param.contents.x, param.contents.y))
        elif wParam == win32con.WM_MOUSEMOVE:
            print("鼠标移动，坐标：x:%d,y:%d" % (param.contents.x, param.contents.y))
        elif wParam == win32con.WM_RBUTTONDOWN:
            print("右键点击，坐标：x:%d,y:%d" % (param.contents.x, param.contents.y))
        elif wParam == win32con.WM_RBUTTONUP:
            print("右键抬起，坐标：x:%d,y:%d" % (param.contents.x, param.contents.y))
